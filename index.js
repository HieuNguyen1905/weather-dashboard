...
document.querySelector(".searchbox").addEventListener("submit", e => {
    ...
    if (!city) {
        return window.alert("Please enter the name of the city")
    }
    ...
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${key}&units=imperial`
        ...
        if (resCur.ok) {
            ...
            return window.alert("City not found");
        }
    })
        .then((dataCur) => {
    ...
    cities.forEach(element => {
        ...
        newCityBtn.setAttribute('id', `${element}`)
        newCityBtn.textContent = element;

    });
    box.append(newCityBtn)
    newCityBtn.addEventListener('click', (e) => {
        const getCity = e.target.id
        console.log(getCity)
        fetch(`https://api.openweathermap.org/data/2.5/weather?q=${getCity}&appid=${key}&units=imperial`
        ).then((resCur) => {
            if (resCur.ok) {
                return resCur.json();
            } else {
